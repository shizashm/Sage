...
class IntakeResult(Base):
    ...
    emotional_intensity: Mapped[int | None] = mapped_column(Integer, nullable=True)  # 1-5
    life_impact_areas: Mapped[list | None] = mapped_column(JSONB, nullable=True)  # ["work", "sleep", ...]
    ...
    group_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("groups.id"), nullable=True)  # set when matched for restart
